...
if (parameters$study == "BIC") {
  ...
  valid.pheno.values <- as.vector(unique(na.omit(phenoTable[, pheno.columns[1]])))
...
for (column in pheno.columns) {
  ...
  !(phenoTable[, column] %in% valid.pheno.values)  <- FALSE
}
...
```{r marker_summary}
pheno..values <- phenoTable[project.samples, pheno.columns]
...
valid.pheno <- phenoTable[valid.samples, pheno.columns]
