...
system.time(dds.vst <- vst(dds0))

## Compute regularized log transformation (takes time)
message("\tDESeq2::rlog()\tComputing regularized log transformation")
system.time(rld <- rlog(dds0)) ## Takes a long time with large number of samples
...
se <- SummarizedExperiment(counts.log2.norm, colData = colData(dds0))
...
if (parameters$run.DESeq2) {
  ...
  system.time(dds0 <- DESeq(dds0))
  
  # Cast the results from DESeq differential expression analysis
  DESeq2.result <- results(dds0, independentFiltering = FALSE)
...
if (parameters$run.edgeR) {
  ...
  PvalueHistogram(Pvalues = edgeR.DEG.table$PValue, main = "edgeR - Distribution of P-values")
...
if (parameters$run.DESeq2) {
  ...
  DESeq2.DEG.table.path <- file.path(export.dir, DESeq2.DEG.table.file)
  if (parameters$gzip.output.files) {
    DESeq2.DEG.table.path <- gzfile(DESeq2.DEG.table.path, "w") ## Compress file
  }
  # dim(counts.log2.norm)
  write.table(x = DESeq2.DEG.table, 
              file = DESeq2.DEG.table.path,
              sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
  if (parameters$gzip.output.files) {
    close(DESeq2.DEG.table.path)
  }
  ...
  DESeq2.DEG.norm.count.path <- file.path(export.dir, DESeq2.DEG.norm.count.file)
  if (parameters$gzip.output.files) {
    DESeq2.DEG.norm.count.path <- gzfile(DESeq2.DEG.norm.count.path, "w") ## Compress file
  }
  # dim(counts.log2.norm)
  write.table(x = counts.log2.norm[DESeq2.DEG, ], 
              file = DESeq2.DEG.norm.count.path,
              sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
  if (parameters$gzip.output.files) {
    close(DESeq2.DEG.norm.count.path)
  }
}
...
if (parameters$run.edgeR) {
  ...
  edgeR.DEG.table.path <- file.path(export.dir, edgeR.DEG.table.file)
  if (parameters$gzip.output.files) {
    edgeR.DEG.table.path <- gzfile(edgeR.DEG.table.path, "w") ## Compress file
  }
  # dim(counts.log2.norm)
  write.table(x = edgeR.DEG.table, 
              file = edgeR.DEG.table.path,
              sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
  if (parameters$gzip.output.files) {
    close(edgeR.DEG.table.path)
  }
  ...
  edgeR.DEG.norm.count.path <- file.path(export.dir, edgeR.DEG.norm.count.file)
  if (parameters$gzip.output.files) {
...
| DESeq2 DEG table | [`r DESeq2.DEG.table.file`](`r DESeq2.DEG.table.file`) |
