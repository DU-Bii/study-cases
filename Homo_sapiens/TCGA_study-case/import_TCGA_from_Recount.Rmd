...
kable(sort(summary(pheno$gdc_cases.project.name), decreasing = TRUE), caption = "Number of samples per TGCA project. ")
```
...
We fuurther discard the samples for which any of the markers is undefined (`NA`), Indeterminate or Equivocal

```{r eval=FALSE}

## Select samples based on project name
...
table(selected.pheno[, "cancer.type"], useNA = "ifany")
...
# dim(selected.counts)
selected.count.file <- paste(sep = "", project.acronym, "_counts-per-gene.tsv")
write.table(x = selected.counts, file = file.path(export.dir, selected.count.file))
