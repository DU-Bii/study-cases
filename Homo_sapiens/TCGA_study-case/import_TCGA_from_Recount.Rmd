...
her2plus <- 
  ...
  pheno[, immuno.columns["HER2"]] == "Positive"
pheno[her2plus, "cancer.type"] <- "HER2+"
...
write.table(x = pheno, file = file.path(export.dir, pheno.file), 
            sep = "\t", quote = FALSE, row.names = FALSE)

## Export counts per gene
count.file <- paste(sep = "", project.acronym, "_counts_all-genes.tsv")
write.table(x = counts.per.gene, file = file.path(export.dir, count.file),
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

## Export the selected counts
filtered.count.file <- paste(sep = "", project.acronym, "_counts_filtered-genes.tsv")
write.table(x = counts.per.gene[filtered.genes, ], file = file.path(export.dir, filtered.count.file),
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
