---
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file ='pavkovic_data_loading_proteome_uuo.html') })
title: "DU-Bii Study cases - Mouse fibrotic kidney - proteomics data"
...
## Download summplementary data from Pavkovic
...
rawValueFile <- file.path(
  outdirs$data,
  paste0("hbc-MouseKidneyFibrOmics-a39e55a/tables/p", 
         parameters$dataset,
         "/results/counts/", 
          parameters$dataset, "_model_counts.csv"))
rawValues <- read.csv(file = rawValueFile, header = 1, row.names = NULL)
rownames(rawValues) <- make.names(rawValues[, "id"], unique = TRUE)
rawValues <- rawValues[, -1]
# names(rawValues)
# unique(sort(as.vector(rawValues[, 1])))
...
sampleMedians <- apply(log2Values, 2, median)
seriesMedian <- median(sampleMedians)
scalingFactors <- seriesMedian / sampleMedians

log2Standardised <- data.frame(matrix(
  nrow = nrow(log2Values),
  ncol = ncol(log2Values)))
colnames(log2Standardised) <- colnames(log2Values)
rownames(log2Standardised) <- rownames(log2Values)
for (j in 1:ncol(log2Values)) {
  log2Standardised[, j] <- log2Values[, j] * scalingFactors[j]
}
