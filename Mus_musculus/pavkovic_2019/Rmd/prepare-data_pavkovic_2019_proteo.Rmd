...
rawCounts <- read.csv(file = rawCountFile, header = 1, row.names = NULL)
rownames(rawCounts) <- make.names(rawCounts[,1], unique = TRUE)

# unique(sort(as.vector(rawCounts[, 1])))
# length(unique(sort(as.vector(rawCounts[, 1]))))
