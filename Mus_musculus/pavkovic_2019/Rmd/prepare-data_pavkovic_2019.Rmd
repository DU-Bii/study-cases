...
dir.create(outdir$data, recursive = TRUE, showWarnings = FALSE)
...
archiveFile <- "MouseKidneyFibrOmics-v1.0"
archiveURL <- file.path("https://zenodo.org/record/2592516/files/hbc", paste0(archiveFile, ".zip"))
localDataArchive <- file.path(outdirs$data, paste0(archiveFile, ".tar"))
...
if (file.exists(localDataArchive)) {
  ...
  download.file(url = transcriptomeSource, destfile =  localDataArchive)
}

## Uncompess the archive
setwd(outdirs$data)
tar(tarfile = paste0(archiveFile, ".tar"))
setwd(outdirs$main)
