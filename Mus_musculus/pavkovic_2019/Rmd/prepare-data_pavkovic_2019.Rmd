...
This however creates a problem when the counts of a given gene in a given sample is 0. To circumvent this, we can add an epsilon before the log2 transformation. 

```{r log2_counts, fig.width=12, fig.height=10, out.width="60%"}

#### Log2 transformatiojn of the counts ####
log2Counts <- log2(rawCounts + epsilon)
...
```{r box plots, fig.width=12, fig.height=10, out.width="60%"}}
#### Box plots ####
par(mar = c(4, 6, 5, 1))
boxplot(log2Counts, horizontal = TRUE, las = 1)
```
...
```{r gene_filtering, fig.width=12, fig.height=10, out.width="60%"}}

undetectedGenes <- apply(rawCounts, MARGIN = 1, FUN = sum) == 0
